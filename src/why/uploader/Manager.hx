...
class Manager<Src, Dest> {
	...
	public static function create<Src, Dest>(local:Local<Src>, remote:Remote<Dest>,
			makeRecorder:(local:Local<Src>, remote:Remote<Dest>) -> Recorder<Src, Dest>):Promise<Manager<Src, Dest>> {
	...
	public inline function initiate(source:Src, destination:Dest):Promise<Upload<Src, Dest>> {
		return remote.makeDestination(destination)
			.next(destination -> new Upload('TODO:uuid', recorder, local.makeSource(source), destination, 0, 5 * 1024 * 1024));
	}
}
